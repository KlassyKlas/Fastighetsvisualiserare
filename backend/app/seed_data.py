"""Exempeldata: välkända svenska infrastrukturprojekt och fiktiva fastigheter.

Fixturerna är typade ingest-modeller — samma kontrakt som externa
datakällor levererar — och skrivs in idempotent via upsert-tjänsterna.
Samma data exporteras till frontendens demo-läge med
``scripts/export_sample_data.py`` så att demodatat aldrig glider
ifrån API:ts form.
"""

from datetime import date
from typing import Any

from app.datasources import InfrastructureProjectIngest, PropertyIngest
from app.domain import ProjectStatus, ProjectType, PropertyType


def _point(lng: float, lat: float) -> dict[str, Any]:
    return {"type": "Point", "coordinates": [lng, lat]}


def _line(*coords: tuple[float, float]) -> dict[str, Any]:
    return {"type": "LineString", "coordinates": [list(c) for c in coords]}


def _rect(center_lng: float, center_lat: float) -> dict[str, Any]:
    """Liten rektangulär tomt (ca 300 × 220 m) kring en mittpunkt."""
    half_lng, half_lat = 0.0015, 0.001
    w, e = center_lng - half_lng, center_lng + half_lng
    s, n = center_lat - half_lat, center_lat + half_lat
    return {
        "type": "Polygon",
        "coordinates": [[[w, s], [e, s], [e, n], [w, n], [w, s]]],
    }


INFRASTRUCTURE_PROJECTS: list[InfrastructureProjectIngest] = [
    InfrastructureProjectIngest(
        external_id="seed-forbifart-stockholm",
        source="manual",
        name="Förbifart Stockholm",
        description=(
            "Motorvägsförbindelse väster om Stockholm, huvudsakligen i tunnel. "
            "Förbinder Kungens Kurva i söder med Häggvik i norr."
        ),
        project_type=ProjectType.TUNNEL,
        status=ProjectStatus.PAGAENDE,
        start_date=date(2015, 1, 1),
        end_date=date(2030, 12, 31),
        budget_sek=34_000_000_000,
        geometry=_line((17.88, 59.39), (17.86, 59.35), (17.84, 59.30), (17.85, 59.25)),
        impact_radius_m=3000,
        metadata_json={
            "trafikverket_id": "TRV2015/1234",
            "kommun": "Stockholm, Järfälla, Sollentuna",
        },
    ),
    InfrastructureProjectIngest(
        external_id="seed-vastlanken",
        source="manual",
        name="Västlänken",
        description=(
            "Järnvägstunnel under centrala Göteborg med tre nya stationer: "
            "Göteborg Central, Haga och Korsvägen."
        ),
        project_type=ProjectType.JARNVAG,
        status=ProjectStatus.PAGAENDE,
        start_date=date(2018, 1, 1),
        end_date=date(2029, 12, 31),
        budget_sek=20_000_000_000,
        geometry=_line((11.93, 57.71), (11.97, 57.715), (12.01, 57.72)),
        impact_radius_m=2000,
        metadata_json={"trafikverket_id": "TRV2014/5678", "kommun": "Göteborg"},
    ),
    InfrastructureProjectIngest(
        external_id="seed-ostlanken",
        source="manual",
        name="Ostlänken",
        description=(
            "Ny järnväg för höghastighetståg mellan Järna och Linköping. "
            "Del av den planerade nya stambanan."
        ),
        project_type=ProjectType.JARNVAG,
        status=ProjectStatus.PLANERAD,
        start_date=date(2024, 1, 1),
        end_date=date(2040, 12, 31),
        budget_sek=90_000_000_000,
        geometry=_line((17.1, 59.0), (16.5, 58.7), (15.6, 58.4)),
        impact_radius_m=5000,
        metadata_json={
            "trafikverket_id": "TRV2017/9012",
            "kommun": "Nyköping, Norrköping, Linköping",
        },
    ),
    InfrastructureProjectIngest(
        external_id="seed-nya-slussen",
        source="manual",
        name="Nya Slussen",
        description=(
            "Ombyggnad av Slussen i Stockholm med ny bussterminal, "
            "gångstråk och förbättrad vattenreglering mellan Mälaren och Saltsjön."
        ),
        project_type=ProjectType.BRO,
        status=ProjectStatus.PAGAENDE,
        start_date=date(2016, 1, 1),
        end_date=date(2027, 12, 31),
        budget_sek=12_000_000_000,
        geometry=_point(18.0717, 59.3195),
        impact_radius_m=1000,
        metadata_json={"kommun": "Stockholm", "stadsdel": "Södermalm"},
    ),
    InfrastructureProjectIngest(
        external_id="seed-norrbotniabanan",
        source="manual",
        name="Norrbotniabanan",
        description=(
            "Ny kustjärnväg mellan Umeå och Luleå. "
            "Avsevärt förbättrar godstransporter och persontrafik i norra Sverige."
        ),
        project_type=ProjectType.JARNVAG,
        status=ProjectStatus.PLANERAD,
        start_date=date(2024, 1, 1),
        end_date=date(2040, 12, 31),
        budget_sek=30_000_000_000,
        geometry=_line((20.26, 63.83), (21.0, 64.75), (22.15, 65.58)),
        impact_radius_m=5000,
        metadata_json={
            "trafikverket_id": "TRV2018/3456",
            "kommun": "Umeå, Skellefteå, Piteå, Luleå",
        },
    ),
    InfrastructureProjectIngest(
        external_id="seed-tvarbana-sodertorn",
        source="manual",
        name="Tvärförbindelse Södertörn",
        description=(
            "Ny väg mellan E4/E20 vid Vårby backe och väg 73 vid Jordbro. "
            "Knyter ihop de södra delarna av Stockholmsregionen."
        ),
        project_type=ProjectType.VAG,
        status=ProjectStatus.PLANERAD,
        start_date=date(2025, 1, 1),
        end_date=date(2032, 12, 31),
        budget_sek=10_000_000_000,
        geometry=_line((17.93, 59.22), (18.04, 59.20), (18.15, 59.19)),
        impact_radius_m=2000,
        metadata_json={"trafikverket_id": "TRV2019/7890", "kommun": "Huddinge, Haninge"},
    ),
    InfrastructureProjectIngest(
        external_id="seed-citybanan",
        source="manual",
        name="Citybanan",
        description=(
            "Pendeltågstunnel under centrala Stockholm mellan Tomteboda och "
            "Stockholms södra. Två nya stationer: Stockholm City och Stockholm Odenplan."
        ),
        project_type=ProjectType.JARNVAG,
        status=ProjectStatus.AVSLUTAD,
        start_date=date(2009, 1, 1),
        end_date=date(2017, 7, 10),
        budget_sek=16_800_000_000,
        geometry=_line((18.04, 59.345), (18.055, 59.335), (18.07, 59.32)),
        impact_radius_m=1500,
        metadata_json={"trafikverket_id": "TRV2006/0001", "kommun": "Stockholm, Solna"},
    ),
    InfrastructureProjectIngest(
        external_id="seed-marieholmstunneln",
        source="manual",
        name="Marieholmstunneln",
        description=(
            "Ny vägtunnel under Göta älv i Göteborg. "
            "Avlastar Tingstadstunneln och förbättrar trafikflödet."
        ),
        project_type=ProjectType.TUNNEL,
        status=ProjectStatus.PAGAENDE,
        start_date=date(2018, 1, 1),
        end_date=date(2026, 12, 31),
        budget_sek=4_000_000_000,
        geometry=_point(12.00, 57.72),
        impact_radius_m=1500,
        metadata_json={"trafikverket_id": "TRV2016/4567", "kommun": "Göteborg"},
    ),
    InfrastructureProjectIngest(
        external_id="seed-roslagsbanan",
        source="manual",
        name="Roslagsbanan utbyggnad",
        description=(
            "Utbyggnad och modernisering av Roslagsbanan i nordöstra Stockholm. "
            "Dubbelspår, nya stationer och ökad kapacitet."
        ),
        project_type=ProjectType.KOLLEKTIVTRAFIK,
        status=ProjectStatus.PLANERAD,
        start_date=date(2023, 1, 1),
        end_date=date(2030, 12, 31),
        budget_sek=6_000_000_000,
        geometry=_line((18.07, 59.36), (18.10, 59.38), (18.17, 59.43)),
        impact_radius_m=1500,
        metadata_json={"kommun": "Stockholm, Täby, Vallentuna"},
    ),
    InfrastructureProjectIngest(
        external_id="seed-e4-sundsvall",
        source="manual",
        name="E4 Förbifart Sundsvall",
        description=(
            "Ny sträckning av E4 förbi Sundsvall med bro över Sundsvallsfjärden. "
            "Minskar genomfartstrafiken i centrala Sundsvall."
        ),
        project_type=ProjectType.VAG,
        status=ProjectStatus.AVSLUTAD,
        start_date=date(2010, 1, 1),
        end_date=date(2015, 10, 19),
        budget_sek=4_100_000_000,
        geometry=_line((17.25, 62.36), (17.30, 62.39), (17.38, 62.42)),
        impact_radius_m=2000,
        metadata_json={"trafikverket_id": "TRV2007/8901", "kommun": "Sundsvall"},
    ),
]


PROPERTIES: list[PropertyIngest] = [
    PropertyIngest(
        designation="Norrmalm 1:5",
        municipality="Stockholm",
        county="Stockholms län",
        area_sqm=4500,
        assessed_value_sek=85_000_000,
        property_type=PropertyType.KONTOR,
        owner_name="Vasakronan AB",
        owner_org_number="556061-4603",
        address="Kungsgatan 10",
        postal_code="111 43",
        city="Stockholm",
        geometry=_rect(18.065, 59.335),
        building_year=1965,
        zoning="C",
    ),
    PropertyIngest(
        designation="Södermalm 3:12",
        municipality="Stockholm",
        county="Stockholms län",
        area_sqm=2800,
        assessed_value_sek=120_000_000,
        property_type=PropertyType.BOSTAD,
        owner_name="Stockholms Kooperativa Bostadsförening",
        owner_org_number="702001-1234",
        address="Hornsgatan 45",
        postal_code="118 49",
        city="Stockholm",
        geometry=_rect(18.07, 59.315),
        building_year=1928,
        living_area_sqm=5200,
        zoning="B",
    ),
    PropertyIngest(
        designation="Solna Centrum 2:1",
        municipality="Solna",
        county="Stockholms län",
        area_sqm=12000,
        assessed_value_sek=350_000_000,
        property_type=PropertyType.HANDEL,
        owner_name="Unibail-Rodamco-Westfield",
        owner_org_number="556079-1415",
        address="Solnavägen 25",
        postal_code="171 45",
        city="Solna",
        geometry=_rect(18.00, 59.36),
        building_year=1995,
        zoning="C",
    ),
    PropertyIngest(
        designation="Nacka Strand 5:3",
        municipality="Nacka",
        county="Stockholms län",
        area_sqm=5000,
        assessed_value_sek=95_000_000,
        property_type=PropertyType.BOSTAD,
        owner_name="JM AB",
        owner_org_number="556045-2770",
        address="Strandvägen 1",
        postal_code="131 52",
        city="Nacka",
        geometry=_rect(18.14, 59.31),
        building_year=2018,
        living_area_sqm=8500,
        zoning="B",
    ),
    PropertyIngest(
        designation="Sundbyberg 1:8",
        municipality="Sundbyberg",
        county="Stockholms län",
        area_sqm=3200,
        assessed_value_sek=78_000_000,
        property_type=PropertyType.BOSTAD,
        owner_name="Sundbybergs Bostäder AB",
        owner_org_number="556055-1234",
        address="Sturegatan 12",
        postal_code="172 31",
        city="Sundbyberg",
        geometry=_rect(17.97, 59.36),
        building_year=1972,
        living_area_sqm=4800,
        zoning="B",
    ),
    PropertyIngest(
        designation="Kista 4:2",
        municipality="Stockholm",
        county="Stockholms län",
        area_sqm=25000,
        assessed_value_sek=420_000_000,
        property_type=PropertyType.KONTOR,
        owner_name="Castellum AB",
        owner_org_number="556475-5550",
        address="Kistagången 8",
        postal_code="164 40",
        city="Stockholm",
        geometry=_rect(17.95, 59.40),
        building_year=2002,
        zoning="J",
    ),
    PropertyIngest(
        designation="Hammarby Sjöstad 7:1",
        municipality="Stockholm",
        county="Stockholms län",
        area_sqm=6000,
        assessed_value_sek=180_000_000,
        property_type=PropertyType.BOSTAD,
        owner_name="HSB Stockholm",
        owner_org_number="504001-1234",
        address="Hammarby Allé 42",
        postal_code="120 63",
        city="Stockholm",
        geometry=_rect(18.10, 59.30),
        building_year=2006,
        living_area_sqm=9200,
        zoning="B",
    ),
    PropertyIngest(
        designation="Bromma 12:4",
        municipality="Stockholm",
        county="Stockholms län",
        area_sqm=800,
        assessed_value_sek=12_500_000,
        property_type=PropertyType.VILLA,
        owner_name="Anna Lindström",
        address="Åkeshovsvägen 18",
        postal_code="168 39",
        city="Stockholm",
        geometry=_rect(17.94, 59.34),
        building_year=1945,
        living_area_sqm=220,
        zoning="B",
    ),
    PropertyIngest(
        designation="Täby Centrum 3:7",
        municipality="Täby",
        county="Stockholms län",
        area_sqm=15000,
        assessed_value_sek=280_000_000,
        property_type=PropertyType.HANDEL,
        owner_name="Unibail-Rodamco-Westfield",
        owner_org_number="556079-1415",
        address="Stora Marknadsvägen 15",
        postal_code="183 34",
        city="Täby",
        geometry=_rect(18.07, 59.44),
        building_year=1968,
        zoning="C",
    ),
    PropertyIngest(
        designation="Flemingsberg 2:5",
        municipality="Huddinge",
        county="Stockholms län",
        area_sqm=8000,
        assessed_value_sek=150_000_000,
        property_type=PropertyType.UTBILDNING,
        owner_name="Akademiska Hus AB",
        owner_org_number="556459-9156",
        address="Alfred Nobels Allé 7",
        postal_code="141 52",
        city="Huddinge",
        geometry=_rect(17.95, 59.22),
        building_year=2012,
        zoning="S",
    ),
]
